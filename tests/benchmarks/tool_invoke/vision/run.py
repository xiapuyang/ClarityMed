...
# Prompts whose YAML versions drive vision-tool behavior. Recorded into
# manifest.json so the Phoenix experiment carries them as searchable
# metadata; unknown names are silently skipped.
_VISION_PROMPT_NAMES = (
    "ask",
    "tool_proposal",
...
async def _main_async(args: argparse.Namespace) -> int:
    ...
    write_snapshot(
        out_dir, build_snapshot(runner="vision", bench_ts=ts, cases=selected)
    )
    ...
    manifest = RunManifest(
        ...
        prompt_versions=collect_prompt_versions(_VISION_PROMPT_NAMES),
