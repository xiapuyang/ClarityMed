"""Skin lesion (ISIC 9-class) dataset download — Kaggle CLI wrapper.

Pulls ``nodoubttome/skin-cancer9-classesisic`` into
``~/.claritymed/data/vision/skin_lesion/``. Mirrors
``ingest/vision/chest_ct/download.py`` — the only differences are the
Kaggle slug and the on-disk subdir name.
...
KAGGLE_SLUG = "nodoubttome/skin-cancer9-classesisic"
# The upstream archive unzips to a top-level directory whose name is
# the full ISIC blurb. Pin the name here so the dataset loader can
# resolve the root without re-scanning the archive shape.
DATASET_SUBDIR = "Skin cancer ISIC The International Skin Imaging Collaboration"


def skin_lesion_data_root() -> Path:
    """Where the skin lesion dataset lives on disk. ``CLARITYMED_HOME`` aware."""
    return _cfg.CLARITYMED_HOME / "data" / "vision" / "skin_lesion"


def is_present(root: Path | None = None) -> bool:
    """Return True iff the dataset has already been extracted."""
    target = (root or skin_lesion_data_root()) / DATASET_SUBDIR
...
def download(root: Path | None = None, *, force: bool = False) -> Path:
    ...
    """
    target_root = root or skin_lesion_data_root()
    target_root.mkdir(parents=True, exist_ok=True)
    if is_present(target_root) and not force:
        logger.info(
            "skin_lesion already extracted at %s; skipping download", target_root
        )
...
def _main(argv: list[str] | None = None) -> int:
    ...
    print(f"skin_lesion extracted at: {out}")
    return 0
